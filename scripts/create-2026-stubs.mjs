/* Create the Aug–Dec 2026 stub budgets (the ones that replace the seller
   budgets in Yardi) for all six properties, linking every data layer.
...
for (const code of ['cwnd', 'drnd', 'lhnd', 'mwnd', 'nrnd', 'rrnd']) {
  ...
  const bv = await call('/budgets', { method: 'POST', body: {
    propertyCode: code, year: 2026, label: `${code} 2026 Budget (from month ${START})`,
  ...
  const t = bv2.tieout;
  console.log(`${code} 2026 m${START}-12 — income ${Math.round(t.egi.budget)} vs prorated UW ${Math.round(t.egi.uw)} (Δ${Math.round(t.egi.variance)}); NOI ${Math.round(t.noi.budget)} vs ${Math.round(t.noi.uw)} (Δ${Math.round(t.noi.variance)})`);
  const noiM = bv2.kpis.monthly.noi;
  console.log(`   monthly NOI Aug..Dec: ${noiM.slice(START - 1).map((v) => Math.round(v)).join(', ')}`);
}
console.log('done');
